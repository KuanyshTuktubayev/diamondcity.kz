						...
						let qryClient = db.query(sqlClient, function(errClient, rowsClient, fieldsClient) {
							...
							if (rowsClient.length == 0) {
								...
								let qryNewClient = db.query(insNewClient, function(errNewClient, resultNewClient, fieldsNewClient) {
									...
									//клиент заводится со стандартным паролем, поэтому надо сразу обновить пароль, записав введенный
									var updNewClPwd = `update Users u
														set u.Pwd = password('${sPwd}')
														where u.ID = ${nNewClientID}`;
									let qryNewClPwd = db.query(updNewClPwd, function(errNewClPwd, resultNewClPwd, fieldsNewClPwd) {
										if (errNewClPwd) { throw errNewClPwd; }
								...
								if (rowsClient[0].Pwd != rowsClient[0].PwdEntered) {
									...
									res.send("<p>Клиент "+nClientID+" "+sClientFIO+" уже существует, но его пароль введен неправильно.</p><p><a href='/'>Вернуться на сайт</a></p>");
									return;
								}
								else {
